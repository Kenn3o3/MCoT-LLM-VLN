...
class LogManager:
    ...
    def start_episode(self, episode_idx: int, instruction: str, subtasks: List[str]) -> None:
    ...
    def set_status(self, status: str) -> None:
            ...
            self.web_ui.update_data(self.current_data)
    def log_inference(self, step_counter: int, current_subtask: str, scene_descriptions: List[str], prompt: str, response: str, action: str, vlm_images: List[Tuple[np.ndarray, np.ndarray]]) -> None:
        """Log inference data including both RGB and depth images."""
        episode_folder = os.path.join(self.log_dir, f"{self.current_time}_episode_{self.current_data['episode_idx']}")
        md_file = os.path.join(episode_folder, "log.md")
        
        # Save RGB and depth images
        image_paths = []
        for i, (rgb, depth) in enumerate(vlm_images):
            rgb_path = os.path.join(episode_folder, f"step_{step_counter}_rgb_{i+1}.jpg")
            cv2.imwrite(rgb_path, rgb)
            depth_colored = self.depth_to_colormap(depth)  # New helper method
            depth_path = os.path.join(episode_folder, f"step_{step_counter}_depth_{i+1}.jpg")
            cv2.imwrite(depth_path, depth_colored)
            image_paths.append((f"step_{step_counter}_rgb_{i+1}.jpg", f"step_{step_counter}_depth_{i+1}.jpg"))
        
        # Append to markdown
        with open(md_file, 'a') as f:
            f.write(f"### Inference at Step {step_counter}\n\n")
            f.write(f"**Current Subtask:** {current_subtask}\n\n")
            ...
            f.write("**Images:**\n")
            for rgb_path, depth_path in image_paths:
                f.write(f"- RGB: ![RGB Image]({rgb_path})\n")
                f.write(f"- Depth: ![Depth Image]({depth_path})\n")
            f.write("\n")
        ...
        self.current_data.update({
            ...
            'reasoning_history': self.current_data['reasoning_history'] + [response],
            'vlm_images': [self.image_to_base64(rgb) for rgb, _ in vlm_images],  # RGB only for simplicity
            'depth_images': [self.image_to_base64(self.depth_to_colormap(depth)) for _, depth in vlm_images]
        })
        self.web_ui.update_data(self.current_data)
